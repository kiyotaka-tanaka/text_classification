...
with graph.as_default():
	...
	#checkpoint_prefix = os.path.join(checkpoint_dir, 'model')
        ...
        label =data_helper.one_hot(label,2)
        data = data_helper.pad_data(data,256,0)
        new_dat = np.array(data)

        x_train,x_dev,y_train,y_dev = train_test_split(data,label,test_size=0.1)

        train_batches = data_helper.batch_iter(list(zip(x_train,y_train)),params['batch_size'],params['num_epochs'],shuffle=True)
        best_accuracy  = 0.0
